...
Observe below the diffs applied on the candidate config buffer. Note that in fact, the target interface gigabitEthernet 0/0/0/2 was not deleted (even though the "no interface" command was issued) but instead the minimal set of changes were automatically applied; i.e. description and IPv4 addresses updated and non-default MTU automatically removed
