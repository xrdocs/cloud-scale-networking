...
This is part two of the blog about the IOS XR’s software architecture. If you missed part 1, it's better to start [with it.](https://xrdocs.io/cloud-scale-networking/blogs/ios-xr-evolution-part1/) In the first part we covered the following concepts: Decoupled Planes Abstraction, Available State/Data Management Patterns, Process Distribution Across Available Compute and High Performance Messaging Infrastructure. 
...
After scalable data partitioning, intelligent process placement to work with that data, high performance messaging infrastructure to enable communication among the processes working with this data, we now explore the next important aspect -  the _data distribution_ and _access patterns_ that applications running in an XR system should use.
...
Many of the data sharing applications would want to know the state of producer(s), i.e. if the producer is active or has gone down so that they can take appropriate action. Again, a good example of this is the interaction between a routing protocol and RIB. If a protocol has populated routes to RIB, RIB should be notified when the protocol goes down so that it can stale the routes, and in the event that the protocol does not come back, it would want to delete those routes and promote backup routes, if any.
